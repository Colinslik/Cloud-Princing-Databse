...
class InfluxDB(object):
    ...
    def get_last(self):
        rows, total = self.list(sort=[("time", "DESC")], limit=1,  count=False)
        return rows, total

    def list(self, condition=None, sort=[], limit=None, offset=0, count=True):
        total = 0
        ...
        values = result.raw.get('series', [{}])[0].get('values', [])
        columns = result.raw.get('series', [{}])[0].get('columns', [])
        rows = []
        for v in values:
            rows.append({item[0]: item[1] for item in zip(columns, v)})
    ...
    def insert(self, tags, fields):
        ...
        self.conn.write_points(json_body, time_precision='ms')
